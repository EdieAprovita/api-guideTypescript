...
describe('Cache Routes', () => {
    ...
    describe('DELETE /api/cache/flush', () => {
        it('should flush cache successfully', async () => {
            ...
            const response = await request(app).delete('/api/cache/flush').expect(200);
        ...
        it('should handle flush errors', async () => {
            ...
            const response = await request(app).delete('/api/cache/flush').expect(500);
    ...
    describe('DELETE /api/cache/invalidate/:pattern', () => {
        it('should invalidate cache pattern successfully', async () => {
            ...
            const pattern = 'user:*';
            const response = await request(app).delete(`/api/cache/invalidate/${pattern}`).expect(200);
        ...
        it('should return error for missing pattern', async () => {
            const response = await request(app).delete('/api/cache/invalidate/').expect(400); // Validation middleware rejects missing pattern
        });
    });
    ...
    describe('DELETE /api/cache/invalidate-tag/:tag', () => {
        it('should invalidate cache by tag successfully', async () => {
            ...
            const tag = 'users';
            const response = await request(app).delete(`/api/cache/invalidate-tag/${tag}`).expect(200);
    ...
    describe('POST /api/cache/monitor/:action', () => {
        it('should start monitoring successfully', async () => {
            ...
            const response = await request(app).post('/api/cache/monitor/start').send({ interval: 10 }).expect(200);
        ...
        it('should stop monitoring successfully', async () => {
            ...
            const response = await request(app).post('/api/cache/monitor/stop').expect(200);
        ...
        it('should handle invalid action', async () => {
            const response = await request(app).post('/api/cache/monitor/invalid').expect(400);
