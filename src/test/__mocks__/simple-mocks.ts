...
export const mockTokenService = {
  generateTokens: vi.fn<[string, string?, string?], Promise<MockTokenPair>>()
    .mockResolvedValue({
      accessToken: 'mock-access-token',
      refreshToken: 'mock-refresh-token'
    }),
  
  verifyAccessToken: vi.fn<[string], Promise<{ userId: string; email: string; role: string }>>()
    .mockResolvedValue({
      userId: 'mock-user-id',
  ...
  verifyRefreshToken: vi.fn<[string], Promise<{ userId: string; email: string; role: string; type: string }>>()
    .mockResolvedValue({
      userId: 'mock-user-id',
      email: 'test@example.com',
      role: 'user',
  ...
  refreshTokens: vi.fn<[string], Promise<MockTokenPair>>()
    .mockResolvedValue({
...
export const mockUserService = {
  registerUser: vi.fn<[Record<string, unknown>], Promise<{ success: boolean; user: MockUser }>>()
    .mockResolvedValue({
      success: true,
      user: {
        _id: 'mock-user-id',
  ...
  loginUser: vi.fn<[Record<string, unknown>], Promise<{ success: boolean; user: MockUser; tokens: MockTokenPair }>>()
    .mockResolvedValue({
      success: true,
      user: {
        _id: 'mock-user-id',
  ...
  getUserById: vi.fn<[string], Promise<MockUser | null>>()
    .mockResolvedValue({
      _id: 'mock-user-id',
      email: 'test@example.com',
      username: 'testuser',
...
export const mockBusinessService = {
  ...
  create: vi.fn<[Partial<MockBusiness>], Promise<MockBusiness>>()
    .mockResolvedValue({
      _id: 'new-business-id',
  ...
  updateById: vi.fn<[string, Partial<MockBusiness>], Promise<MockBusiness>>()
    .mockResolvedValue({
      _id: 'mock-business-id',
...
export const mockCacheService = {
  ...
  getStats: vi.fn<[], Promise<MockCacheStats>>()
    .mockResolvedValue({
      hitRatio: 85,
      memoryUsage: '25M',
      cacheSize: 50,
...
export const mockCacheAlertService = {
  ...
  getConfig: vi.fn<[], MockAlertConfig>()
    .mockReturnValue({
      enabled: true,
  ...
  getMonitoringStatus: vi.fn<[], Record<string, unknown>>()
    .mockReturnValue({
      enabled: true,
      running: false,
      lastCheck: new Date(),
...
export const createMockModel = (modelName: string) => ({
  create: vi.fn<[Record<string, unknown>], Promise<MockMongooseDocument>>()
    .mockResolvedValue({
      _id: `mock-${modelName.toLowerCase()}-id`,
      toObject: () => ({ _id: `mock-${modelName.toLowerCase()}-id` })
    }),
  
  find: vi.fn<[Record<string, unknown>?], Promise<MockMongooseDocument[]>>()
    .mockResolvedValue([]),
  
  findById: vi.fn<[string], Promise<MockMongooseDocument | null>>()
    .mockResolvedValue({
      _id: `mock-${modelName.toLowerCase()}-id`,
      toObject: () => ({ _id: `mock-${modelName.toLowerCase()}-id` })
    }),
  
  findOne: vi.fn<[Record<string, unknown>], Promise<MockMongooseDocument | null>>()
    .mockResolvedValue({
      _id: `mock-${modelName.toLowerCase()}-id`,
      toObject: () => ({ _id: `mock-${modelName.toLowerCase()}-id` })
    }),
  
  findByIdAndUpdate: vi.fn<[string, Record<string, unknown>, Record<string, unknown>?], Promise<MockMongooseDocument | null>>()
    .mockResolvedValue({
      _id: `mock-${modelName.toLowerCase()}-id`,
      toObject: () => ({ _id: `mock-${modelName.toLowerCase()}-id` })
    }),
  
  deleteOne: vi.fn<[Record<string, unknown>], Promise<{ deletedCount: number }>>()
    .mockResolvedValue({ deletedCount: 1 }),
  
  deleteMany: vi.fn<[Record<string, unknown>], Promise<{ deletedCount: number }>>()
    .mockResolvedValue({ deletedCount: 1 }),
