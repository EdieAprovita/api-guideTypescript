...
describe('Cache Middleware Tests', () => {
    ...
    describe('Cache Key Generation', () => {
        ...
        it('should handle query parameters in cache key', async () => {
            ...
            expect(mockedCacheService.get).toHaveBeenCalledWith('GET:/cached-route/123?filter=active&sort=name');
        });
    });
    ...
    describe('Cache Invalidation', () => {
        it('should not cache POST requests', async () => {
            const response = await request(app).post('/invalidate/123').send({ data: 'test' });
