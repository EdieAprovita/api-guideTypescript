...
import { validateErrorResponse, validateValidationErrorResponse, HTTP_STATUS } from '../utils/commonTestHelpers';
...
const expectValidationErrorResponse = (
    response: request.Response,
    expectedMessage: string,
    expectedError: string,
    errors: unknown[]
...
app.get('/validation-error', (_req, _res, next) => {
    ...
    error.errors = [
        ...
        { field: 'password', message: testConfig.messages.validation.passwordLength },
    ];
    next(error);
});
    ...
    describe('Mongoose Validation Errors', () => {
        it('should handle ValidationError with field details', async () => {
            ...
            expectValidationErrorResponse(response, 'Validation Error', 'Invalid input data', [
                {
                    field: 'email',
                    message: testConfig.messages.validation.emailFormat,
                },
                {
                    field: 'password',
                    message: testConfig.messages.validation.passwordLength,
                },
            ]);
        });
